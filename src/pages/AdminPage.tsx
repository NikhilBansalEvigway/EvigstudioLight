...
export default function AdminPage() {
  ...
  const loadAudit = useCallback(async () => {
    ...
    if (!r.ok) return;
    const d = (await r.json()) as { logs: AuditRow[]; total: number };
    setAudit(d.logs);
    setAuditTotal(d.total);
  }, [auditPage, auditPageSize, auditQuery]);
  ...
  useEffect(() => {
    ...
    void loadAudit();
  }, [user, serverAvailable, user.role, loadAudit]);
        ...
        <TabsContent value="audit" className="space-y-2">
          {user.role === 'admin' && (
            <div className="relative max-w-md">
              <Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
              <Input
                placeholder="Search audit (action, resource…)"
          ...
          <div className="flex justify-end">
            <Button variant="outline" size="sm" type="button" onClick={() => void loadAudit()}>
              Refresh
            </Button>
          </div>
          <div className="border border-border rounded-md max-h-[480px] overflow-auto text-[11px] font-mono">
            {audit.map((log) => (
              <div key={log.id} className="border-b border-border p-2 hover:bg-secondary/30">
                <div className="text-muted-foreground">{log.createdAt}</div>
                <div>
                  <span className="text-primary">{log.action}</span> <span>{log.resourceType}</span>
                  {log.resourceId && <span className="text-muted-foreground"> #{log.resourceId}</span>}
                </div>
                {log.metadata != null && (
                  <pre className="text-[10px] text-muted-foreground whitespace-pre-wrap break-all mt-1">
                    {JSON.stringify(log.metadata)}
                  </pre>
                )}
              </div>
            ))}
            {audit.length === 0 && <div className="p-4 text-muted-foreground">No entries.</div>}
          </div>
        </TabsContent>
      </Tabs>
